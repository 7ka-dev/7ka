      ...
      {SECTIONS.map((s) => {
        ...
        return (
          <button
            ...
            className="flex items-center gap-3 px-3 py-2.5 text-left cursor-pointer"
            style={{
              fontSize: '0.95rem',
              letterSpacing: '0.08em',
              color: isActive ? '#f0c040' : '#c8a84b',
            ...
            onMouseEnter={(e) => {
              if (!isActive) {
                ...
                e.currentTarget.style.background = '#140f05'
              }
            }}
            onMouseLeave={(e) => {
              if (!isActive) {
          ...
          >
            <span style={{ color: isActive ? '#8a7040' : '#5a4a2a' }}>[{s.key}]</span>
            {s.label}
          </button>
        )
      })}
