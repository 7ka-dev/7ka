...
const COMMANDS: CommandBlock[] = [
  {
    ...
    output: [
      ...
      o('Last login: <r>▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓</r> from <r>▓▓▓▓▓▓▓▓▓▓▓▓▓</r>', T.NORMAL),
      o('──────────────────────────────────────────', T.INSTANT),
    ],
  },
  {
  ...
  {
    command: 'ssh unit7',
    output: [o('INITIATING CONNECTION TO <g>UNIT-7 MAINFRAME</g>...', T.NORMAL)],
    onComplete: (): void => {
      useAppStore.getState().toMainframe()
    },
  },
  {
    command: 'help',
  ...
  { command: 'sudo', output: [o('<e>Permission denied.</e> <d>(nice try)</d>')] },
  { command: 'pwd', output: [o('/home/guest')] },
  { command: 'whoami', output: [o('<d>good question. existential, even.</d>')] },
  { command: 'rm -rf /', output: [o("<e>we see what you're trying to do.</e> <d>AGENT-0010 also tried this.</d>")] },
  { command: 'rm', output: [o('<e>absolutely not.</e>')] },
  { command: 'hack', output: [o("<g>HACKING...</g> <d>you're already in.</d>")] },
  { command: 'hello', output: [o('<g>hello.</g> <d>we see you.</d>')] },
  { command: 'hello world', output: [o('<g>hello world.</g> <d>classic.</d>')] },
  { command: 'matrix', output: [o("<g>follow the white rabbit.</g> <d>we don't have one.</d>")] },
  { command: 'coffee', output: [o('<g>BREWING...</g> <d>this entire org runs on coffee.</d>')] },
  { command: '007', output: [o('<w>AGENT-007 DOES NOT EXIST.</w> <d>stop asking.</d>')] },
  { command: 'exit', output: [o("<d>you can't leave. you're already here.</d>")] },
  { command: 'vim', output: [o("<d>to exit vim: <g>:q!</g> — you're welcome.</d>")] },
  { command: 'uname', output: [o('UNIT7-OS 0.7.4-unstable #1 SMP x86_64')] },
  { command: 'uptime', output: [o('<d>uptime: long enough to regret it.</d>')] },
  {
    command: 'top',
    output: ol(
      [
...
export type ResolvedCommand = { type: 'block'; block: CommandBlock } | { type: 'unknown'; input: string }
...
export function resolveCommand(raw: string): ResolvedCommand {
  ...
  // ssh matches any ssh/connect variant
  if (lower.startsWith('ssh') || lower.startsWith('connect')) {
    const sshBlock = COMMANDS.find((c) => c.command === 'ssh unit7')
    if (sshBlock) return { type: 'block', block: sshBlock }
  }
...
export function makeTerminalLine(text: string, isCommand = false): TerminalLine {
